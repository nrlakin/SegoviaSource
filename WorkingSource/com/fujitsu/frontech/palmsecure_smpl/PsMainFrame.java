...
import com.fujitsu.frontech.palmsecure_smpl.segovia.RecipientDisplay;
import com.fujitsu.frontech.palmsecure_smpl.segovia.SegoviaLogoLabel;
...
public class PsMainFrame extends JFrame implements ActionListener, WindowListener, PsBusinessListener {
	...
	private static final String SEGOVIA_LOGO = "segoviaband.bmp";
	...
	private JLabel lblVillage = null;
	private JLabel lblAge = null;
	private JLabel lblRoofing = null;
	//private JLabel lblRoofing = null;
	...
	private SegoviaLogoLabel lblLogo = null;
	
	private PsInputIdText name = null;
	private PsInputIdText firstName = null;
	private PsInputIdText location = null;
	private PsInputIdText village = null;
	private PsInputIdText age = null;
	//private PsInputIdText roofing = null;
	

	private long usingGuideMode = 0;
	private long usingSensorType = 0;
	private long usingSensorExtKind = 0;
	private String campaign = "CT201412";
	...
	public PsMainFrame() {
		...
		lblLogo = new SegoviaLogoLabel();
		lblLogo.setBorder(null);
		lblLogo.setBounds(12, 10, 760, 35);
		...
		pnlID.setBounds(393, 55, 246, 309);		// Stretch original panel to bottom of old idlist panel
		...
		lblFirst.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		lblFirst.setBounds(1, 15, 84, 33);
		pnlID.add(lblFirst);
		
		// Text(input):first name
		firstName = new PsInputIdText();
		firstName.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		firstName.setText("1234567890123456");
		firstName.setBounds(92, 15, 142, 33);
		pnlID.add(firstName);
		firstName.setColumns(10);

		// Label:Last name
		lblID = new JLabel("Last:");
		lblID.setHorizontalAlignment(SwingConstants.RIGHT);
		lblID.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		lblID.setBounds(1, 63, 84, 33);
		pnlID.add(lblID);
		
		// Text(input):Name
		name = new PsInputIdText();
		name.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		name.setText("1234567890123456");
		name.setBounds(92, 63, 142, 33);
		pnlID.add(name);
		name.setColumns(10);

		// Label:Village
		lblVillage = new JLabel("Village:");
		lblVillage.setHorizontalAlignment(SwingConstants.RIGHT);
		lblVillage.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		lblVillage.setBounds(1, 111, 84, 33);
		pnlID.add(lblVillage);
		
		// Text(input):village
		village = new PsInputIdText();
		village.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		village.setText("1234567890123456");
		village.setBounds(92, 111, 142, 33);
		pnlID.add(village);
		...
		lblLocation.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		lblLocation.setBounds(1, 159, 84, 33);
		pnlID.add(lblLocation);
		
		// Text(input):family size
		location = new PsInputIdText();
		location.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		location.setText("1234567890123456");
		location.setBounds(92, 159, 142, 33);
		pnlID.add(location);
		...
		lblAge.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		lblAge.setBounds(1, 207, 84, 33);
		pnlID.add(lblAge);
		
		// Text(input):roofing
		age = new PsInputIdText();
		age.setFont(new Font(PS_FONT_NAME, Font.PLAIN, 16));
		age.setText("1234567890123456");
		age.setBounds(92, 207, 142, 33);
		pnlID.add(age);
		age.setColumns(10);
		...
		btnIdentify.setBounds(651, 217, PS_BTN_WIDTH, PS_BTN_HEIGHT);
		getContentPane().add(btnIdentify);
		
		// Button:Export
		btnExport = new JButton("Export");
		btnExport.setFont(buttonFont);
		btnExport.setBounds(651, 285, PS_BTN_WIDTH, PS_BTN_HEIGHT);
	...
	private void ReadLogoBmp() throws PsAplException {

		logoImage = null;

		File logoFile = new File(SEGOVIA_LOGO);
	...
	private void SetSegoviaLogoBmp() {
		if (logoImage != null) {
			lblLogo.SetLogo(logoImage);
			lblLogo.repaint();
		}
	}
	...
	/*
	 * Note that this method initializes internal data structure as well as GUI element! 
	...
	private void Ps_Sample_Apl_Java_InitIdList() throws PsAplException {
		...
		dataMng.Ps_Sample_Apl_Java_Init(usingSensorType, usingGuideMode, campaign);
	...
	//TODO: this is the method in which we need to handle the other fields
	private void Ps_Sample_Apl_Java_ActionEnroll() {

		PsDataManager dataMng = PsDataManager.GetInstance();
		String gdid = dataMng.nextGDID();
		Recipient recipient = new Recipient(gdid);

		Ps_Sample_Apl_Java_SetWorkMessage(PsFileAccessorLang.Guidance_WorkEnrollStart);
		Ps_Sample_Apl_Java_SetGuidance("", false);
		Ps_Sample_Apl_Java_DisplayGuideBmp();

		recipient.update("lastName", name.getText());
		recipient.update("firstName", firstName.getText());
		recipient.update("village", village.getText());
		recipient.update("age", age.getText());
		recipient.update("location", location.getText());
		recipient.update("recipient_name", firstName.getText() + " " + name.getText());

		if (recipient.get("firstName").compareTo("") == 0) {
		...
		if (dataMng.Ps_Sample_Apl_Java_IsRegist(gdid)) {
			// Will always be false, got next avail gdid.
		...
		PsThreadEnroll enrollThread = new PsThreadEnroll(this, this, palmsecureIf, ModuleHandle, recipient);
		enrollThread.start();
		...
		try {
			if (stResult.result == PalmSecureConstant.JAVA_BioAPI_OK
				...
				Ps_Sample_Apl_Java_SetGuidance(
						...
						false,
						stResult.userId.get(0));
				// Show message dialog with info
				
				RecipientDisplay.showRecipientDialog(this,stResult.recipient);
